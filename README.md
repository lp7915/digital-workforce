...
当前 `public/` 已改为独立前端演示：数字员工、项目、运行中的任务三个模块，浏览器本地保存数据，不调用原业务 API。打开 http://127.0.0.1:8790 即可预览，不需要登录。原服务端代码与数据库未修改；以下后端说明属于原实现，不能视为新前端已完成联调。

员工详情覆盖基础信息、身份、知识与规则、技能、记忆、环境、凭证引用、飞书及豆包、版本控制。项目详情覆盖项目记忆、项目群聊、成员及记忆改写权限。交互与后端接入边界见 `outputs/前端交付说明.md`。
